...
export default function ProductPage() {
  ...
  if (loading) {
    return <div className="container-page py-20 text-center text-muted">Загрузка...</div>;
  }
  ...
  const features = [
    ...
    "Премиальная ткань",
    "Подкладка и детали под заказ",
    "Сохранение конфигурации в кабинете",
  ];
    ...
    <div className="container-page py-14">
      ...
      <div className="grid gap-10 lg:grid-cols-2">
          ...
          >
            {product.image ? (
              <div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-bg/50">
                <img src={product.image} alt={product.fullName} className="h-full w-full object-cover" />
                {product.tag && (
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <div className="aspect-square rounded-lg border border-gold/20" style={{ background: color }} />
              <div className="aspect-square rounded-lg border border-gold/20" style={{ background: accent }} />
              <div className="flex aspect-square items-center justify-center rounded-lg border border-gold/20 bg-bg/40">
                <span className="text-xs uppercase tracking-wider text-gold">{pattern}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <h1 className="mt-5 font-serif text-5xl font-light leading-tight">{product.fullName}</h1>

          <p className="mt-5 text-lg leading-8 text-muted">{product.description}</p>

          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            <FeatureStat label="Ткань" value={product.fabric} />
            <FeatureStat label="Срок пошива" value="4-6 недель" />
            <FeatureStat label="Категория" value={product.style} />
            <FeatureStat label="Рейтинг" value={`★ ${product.rating}`} />
          </div>

          <div className="mt-8 border-t border-gold/15 pt-6">
            <p className="font-serif text-5xl text-gold">{formatPrice(product.price)}</p>
            {product.oldPrice && <p className="mt-1 text-muted line-through">{formatPrice(product.oldPrice)}</p>}
          </div>
